...
class Game_Client < EventMachine::Connection
  ...
  def has_seal_equip?(item)
	...
	def change_exp(exp)
		...
		if @level > last_level
			$network.player_chat_message(self, sprintf(LevelUp, last_level, @level), Configs::ALERT_COLOR)
			recover_all
		end
	end
  ...
  def max_floor_damage
	...
	def lose_exp(exp)
		gain_exp(-exp)
		$network.player_chat_message(self, exp == 0 ? NotLoseExp : sprintf(Died, exp.to_s.reverse.scan(/...|..|./).join('.').reverse), Configs::ERROR_COLOR)
	end
	...
	def learn_skill(skill_id)
		...
		$network.player_chat_message(self, "#{LearnedSkill} #{$data_skills[skill_id].name}.", Configs::SUCCESS_COLOR)
	end
  ...
  def gold_rate
	...
	def accept_guild
		...
		$network.guild_chat_message(self, "#{@name} #{JoinGuild} #{@guild_name}.", Configs::SUCCESS_COLOR)
	...
	def leave_guild
		$network.guild_chat_message(self, "#{@name} #{LeaveGuild} #{@guild_name}.", Configs::ERROR_COLOR)
	...
	def add_itens_count(item)
		@quests.each_value do |quest|
			...
			$network.player_chat_message(self, "#{Have} #{item_number(item)}/#{quest.item_amount} #{item.name}.", Configs::SUCCESS_COLOR)
			break
		end
	end
	...
	def start_map_event(x, y, triggers, normal)
		...
		$network.maps[@map_id].events_xy(x, y).each do |event|
			event.start(self) if !event.enemy? && event.trigger_in?(triggers) && event.normal_priority? == normal
		end
	end
