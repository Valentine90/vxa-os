  ...
  def load_game_data
	...
	def load_skills
		...
		(1...skills.size).each do |skill_id|
			...
			$data_skills[skill_id].level = Note.read_number('Level', skills[skill_id].note)
			# Obtém o índice da animação de ataque ou um valor nulo, diferentemente do read_number
			#que retorna 0 se não existir nenhum valor no banco de dados
	...
	def load_banlist
		...
		begin
			...
			puts("Erro: #{e}".colorize(:red))
		end
	end

	def load_global_switches
		return unless File.exist?('Data/switches.json')
		puts('Carregando switches globais...')
		data = JSON.parse(File.read('Data/switches.json'))
		@switches = Game_GlobalSwitches.new(data)
	end
	...
	def save_global_switches
		file = File.open('Data/switches.json', 'wb')
		file.write(JSON.generate(@switches.data))
		file.close
	end
