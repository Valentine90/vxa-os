...
class Game_Client < EventMachine::Connection
  ...
  def feature_objects
	...
	def full_inventory?(item)
		...
		if size == Configs::MAX_PLAYER_ITEMS && !has_item?(item)
			...
			$server.alert_message(self, Enums::Alert::FULL_INV)
			return true
		end
		return false
	end
  ...
  def has_seal_equip?(item)
	...
	def add_new_state(state_id)
		super
		$server.send_player_state(self, state_id)
	end

	def remove_state(state_id)
		super
		$server.send_player_state(self, state_id, false)
	end

	def add_buff(param_id)
		super
		$server.send_player_buff(self, param_id, 1)
	end
	...
	def erase_buff(param_id)
		super
		$server.send_player_buff(self, param_id, 0)
	end
	...
	def change_hotbar(id, type, item_id)
		...
		$server.send_player_hotbar(self, id)
	end
	...
	def change_exp(exp)
		...
		if @level > last_level
			$server.player_message(self, sprintf(LevelUp, last_level, @level), Configs::ALERT_COLOR)
			recover_all
		end
	end
	...
	def check_floor_effect
		execute_floor_damage if $server.maps[@map_id].damage_floor?(@x, @y)
	end
  ...
  def max_floor_damage
	...
	def lose_exp(exp)
		gain_exp(-exp)
		$server.player_message(self, sprintf(Died, exp.to_s.reverse.scan(/...|..|./).join('.').reverse), Configs::ERROR_COLOR)
	end
	...
	def forget_skill(skill_id)
		...
		$server.send_player_skill(self, skill_id, false)
	end
  ...
  def gold_rate
	...
	def open_create_guild
		...
		if in_guild?
			$server.alert_message(self, Enums::Alert::IN_GUILD)
			return
		end
		@creating_guild = true
	...
	def accept_guild
		...
		$server.guild_message(self, "#{@name} #{JoinGuild} #{@guild}.", Configs::SUCCESS_COLOR)
	...
	def start_quest(quest_id)
		...
		$server.send_add_quest(self, quest_id)
	end
