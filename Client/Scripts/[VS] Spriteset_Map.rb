...
class Spriteset_Map
  ...
  def create_objects
    ...
    $game_map.drops.each { |drop| @drop_sprites << Sprite_Drop.new(@viewport1, drop) }
  end
  ...
  def update_projectiles
    @projectile_sprites.each_with_index do |projectile, i|
      if projectile.destroy?
        $game_map.projectiles.delete_at(i)
        projectile.bitmap.dispose
        projectile.dispose
  ...
  def update_target
    if $game_player.has_target? && $game_player.target.actor?
      create_target unless @target_sprite
  ...
  def create_target
    ...
    @target_sprite.oy = @target_sprite.bitmap.height / 2 + 20
  ...
  def create_admin_message
    ...
    @message_sprite.bitmap = Bitmap.new(400, 50)
    @message_sprite.bitmap.font.color.set(Color::Yellow)
    @message_sprite.bitmap.font.size = 22
    @message_sprite.bitmap.font.bold = true
    $windows[:panel].word_wrap($admin_msg, @message_sprite.bitmap.width - 8).each_with_index do |text, i|
      @message_sprite.bitmap.draw_text(0, 24 * i, @message_sprite.bitmap.width, 24, text, 1)
    end
